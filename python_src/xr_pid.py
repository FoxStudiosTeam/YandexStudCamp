...
# @version: python3.7
# @Author  : xiaor
# @Explain : Настройка PID регуляторов
# @Time    : 2020/05/09
# @File    : xr_pid.py
# @Software: PyCharm
...
class PID:
    ...
    def update(self, feedback_value):
        ...
        if delta_time >= self.sample_time:
            self.PTerm = self.Kp * self.error  # Пропорциональная составляющая
            self.ITerm += self.error * delta_time  # Интегральная составляющая
